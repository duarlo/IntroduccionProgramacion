...
function convertDollarToReal(dollarValue: number): number {
...
function displayPrimeNumbers(limit: number): void {
...
const rectangleAreaAndPerimeter = (h: number, l: number) =>{
    let area = h * l;
    let perimeter = 2 * (h + l);
    console.log(`Area: ${area} u2, Perimeter: ${perimeter} u`);
}
//Crea una función que muestre en pantalla el área y el perímetro de una sala circular, utilizando su radio que se proporcionará como parámetro. Considera Pi = 3,14.
const circularAreaAndPerimeter = (r:number) => {
